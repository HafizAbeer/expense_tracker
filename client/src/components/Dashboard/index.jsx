...
const Dashboard = ({ user, setUser, logout, token }) => {
  ...
  const fetchExpenses = async () => {
    try {
      const res = await fetch(
        "https://expense-tracker-qhlb.vercel.app/api/expenses",
        {
  ...
  const updateExpense = async (id, updatedData) => {
    try {
      const res = await fetch(
        `https://expense-tracker-qhlb.vercel.app/api/expenses/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteExpense = async () => {
    try {
      const res = await fetch(
        `https://expense-tracker-qhlb.vercel.app/api/expenses/${confirmDeleteId}`,
        {
          method: "DELETE",
          headers: { "x-auth-token": token },
        },
  ...
  const updateBudget = async (newBudget) => {
    try {
      const res = await fetch(
        "https://expense-tracker-qhlb.vercel.app/api/auth/budget",
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="container" style={{ padding: "2rem 0" }}>
      ...
      <div className="dashboard-grid">
        ...
        <div className="glass-card" style={{ height: "fit-content" }}>
          ...
          <ExpenseList
            expenses={filteredExpenses}
            deleteExpense={(id) => setConfirmDeleteId(id)}
